...
public class PPGHRDet {
	...
	private static int HR_AVG_WINDOW_SAMPLES = 5 * 75;
	private static int HISTORYBUFFER_LENGTH = 16; // Size must be power of 2 so simple math for calculating history index
	...
	private PeakDetect mPeakDetect = new PeakDetect();
	...
	public PPGHRDet() {
		mSampleRate = PulseOximeter.SAMPLING_RATE; // 300Hz (OESA beat detector assumes 300Hz)
		mADCUnit = 50;  // 50 units per mV
		mADCZero = 0; // 128. (8bit unsigned data, where 128 = 0mV)
		reset();
	}
	...
	public int process(int ecgSample) {
		...
		delay = mPeakDetect.process(ecgSample);
