...
public class PeakDetect {
	public static final int SAMPLE_RATE = PulseOximeter.SAMPLING_RATE;// Sample rate in Hz
	
	private static final String TAG = "PeakDetect";
    ...
    private static final int HPBUFFER_LGTH = MS80;
    private static final int PRE_BLANK = MS195;
    private static final int MIN_PEAK_AMP = 5; // Prevents detections of peaks smaller than about 300 uV.
    
    /// Detector threshold  = 0.3125 = TH_NUMERATOR/TH_DENOMINATOR
    private static final int TH_NUMERATOR = 1000;
    private static final int TH_DENOMINATOR = 10000;
    private static final int WINDOW_WIDTH = MS80;           // Moving window integration width.
    private static final int FILTER_DELAY = (int)  (((double) LPBUFFER_LGTH / 2 - 1) + ((double) DERIV_LENGTH / 2)  + PRE_BLANK); //+ + (((double) HPBUFFER_LGTH - 1) / 2)  filter delays plus 200 ms blanking delay
    ...
    public int process(int datum) {
        ...
        mDDBuffer[mDDPtr] = mDeriv1.addSample(datum);
        if (++mDDPtr == DER_DELAY) {
            mDDPtr = 0;
        }
