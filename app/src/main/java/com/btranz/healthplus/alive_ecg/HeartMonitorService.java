...
public class HeartMonitorService extends Service implements HeartMonitorListener {
    ...
    private void startForeground() {
        ...
        NotificationCompat.Builder mBuilder =
        		...
        		.setOngoing(true)
                .setSmallIcon(R.drawable.ic_hm_running)
    ...
    public class HMBinder extends Binder {
    	public HeartMonitorService getService() {
    ...
    public void onAliveStatus(int statusID) {
    	mStatus = statusID;
    	if(mPause) return;
        if (statusID == HeartMonitorStatus.HM_STATUS_NONE)
            stop();
    	synchronized(this) {
    		if(mHeartMonitorListener !=null) {
    ...
    public void addECGdata(double reading)
    {
        ecgData.add(reading);
    }

    public ArrayList<Double> getECGdata(){return ecgData;}
}
